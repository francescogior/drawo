...
const drawingDiffSumX = (drawing: Drawing) => drawingDiffsX(drawing).reduce(sum, 0)
const drawingDiffSumY = (drawing: Drawing) => drawingDiffsY(drawing).reduce(sum, 0)

const drawingAvgDiffX = (drawing: Drawing) => drawingDiffSumX(drawing) / drawingLength(drawing)
const drawingAvgDiffY = (drawing: Drawing) => drawingDiffSumY(drawing) / drawingLength(drawing)
...
export default function mapProps({ drawings, points }: Props): UIProps {
  ...
  // const avgMaxX = round(drawings.map(drawingMaxDiffX).reduce(sum, 0) / drawingsCounter)
  // const maxAvgX = round(max(...drawings.map(drawingAvgDiffX)))
  const avgAvgX = round(drawings.map(drawingAvgDiffX).reduce(sum, 0) / drawingsCounter)

  const maxMaxY = round(max(...drawings.map(drawingMaxDiffY)))
  // const avgMaxY = round(drawings.map(drawingMaxDiffY).reduce(sum, 0) / drawingsCounter)
  // const maxAvgY = round(max(...drawings.map(drawingAvgDiffY)))
  const avgAvgY = round(drawings.map(drawingAvgDiffY).reduce(sum, 0) / drawingsCounter)
